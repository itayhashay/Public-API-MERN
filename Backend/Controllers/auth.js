...
router.post("/login", async (req, res) => {
  try {
    ...
    const isLoggedIn = await firebaseService.login(
      req.body.username,
      req.body.password
    );
    if (!isLoggedIn) {
    ...
    res
      ...
      .send({ error: "An error occured while logging in" });
  }
});
...
router.post("/refresh", async (req, res) => {
  try {
    ...
    if (err.name === "TokenExpiredError") {
      return res
        .status(StatusCodes.UNAUTHORIZED)
        .send({ error: "Refresh token has expired" });
    }

    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .send({ error: "An error occured while refreshing the token" });
  }
});
...
router.post("/signup", async (req, res) => {
  try {
    const { username, firstName, lastName, gender, birthday, email, password } =
      req.body;
    ...
    const isRegisterdOnFirebase = await firebaseService.register(
      username,
      password
    );
